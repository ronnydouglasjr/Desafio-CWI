...
public class Simulador {
  ...
  public void simular() throws SauronDominaOMundoException {
    ...
    while (verificador) {
      ...
      for (int i = 0; i < 10; i++) {
        Personagem p = mapa.buscarPersonagem(i);
      ...
      Personagem personagem = mapa.buscarPersonagem(atual.intValue());
      if (personagem != null && personagem.getConstituicao() > 0) {
        System.out.println("-----------Rodada-----------");
        System.out.println("Mapa: " + mapa.exibirTabuleiro());
      ...
      for (int t = 0; t < 10; t++) {
        Personagem p = mapa.buscarPersonagem(t);
        if (p != null && p.getConstituicao() <= 0) {
          mapa.removerPersonagem(t);
        }
      }
